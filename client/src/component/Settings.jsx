...
function Settings({closeSetting, handleDarkMode, darkMode}) {
    const close = () => {
        closeSetting(false)
    }
    return (
        <div className='fixed w-full h-full top-0 w-97 flex justify-center items-center bg-black bg-opacity-60'>
            ...
            <div className={`${darkMode ? 'bg-black text-white' : 'bg-white text-black' } w-3/5 h-3/5 rounded-3xl flex p-8`}>
                <div className='flex h-8 w-full justify-between'>
                    <span>Dark Mode</span>
                    <ToggleButton handleDarkMode={handleDarkMode}/>
                </div>
            </div>
        </div>
    )
